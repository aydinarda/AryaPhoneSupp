...
with tab_policy:
    ...
    # Discrete policy settings:
    #   Low  -> 1
    #   Mid  -> 5
    #   High -> 10
    LEVEL_MAP = {"Low": 1.0, "Mid": 5.0, "High": 10.0}
    LEVEL_OPTIONS = list(LEVEL_MAP.keys())

    def _nearest_level(val) -> str:
        try:
            v = float(val)
        except Exception:
            v = 5.0
        return min(LEVEL_MAP.items(), key=lambda kv: abs(v - kv[1]))[0]

    def _level_select(label: str, current_val) -> float:
        default = _nearest_level(current_val)
        choice = st.selectbox(label, LEVEL_OPTIONS, index=LEVEL_OPTIONS.index(default))
        return float(LEVEL_MAP[choice])

    def _yes_no(label: str, current_val) -> float:
        try:
            v = float(current_val)
        except Exception:
            v = 0.0
        default_idx = 1 if v >= 0.5 else 0  # ["No", "Yes"]
        choice = st.radio(label, ["No", "Yes"], index=default_idx, horizontal=True)
        return 1.0 if choice == "Yes" else 0.0

    c1, c2, c3 = st.columns(3)
    with c1:
        p.env_mult = _level_select("Environmental multiplier", p.env_mult)
        p.social_mult = _level_select("Social multiplier", p.social_mult)
        p.cost_mult = _level_select("Cost multiplier", p.cost_mult)

    with c2:
        p.strategic_mult = _level_select("Strategic multiplier", p.strategic_mult)
        p.improvement_mult = _level_select("Improvement multiplier", p.improvement_mult)
        p.low_quality_mult = _level_select("Low-quality multiplier", p.low_quality_mult)

    with c3:
        p.child_labor_penalty = _yes_no("Child labor penalty", p.child_labor_penalty)
        p.banned_chem_penalty = _yes_no("Banned chemicals penalty", p.banned_chem_penalty)
