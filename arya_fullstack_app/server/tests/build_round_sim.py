...
cells.append(code(
    ...
    "    )\n"
    "\n"
    "def run_round(team_specs, alpha=3.0, beta=3.0, delta=0.1, u_outside=None,\n"
    "              cl_penalty=0.0, bc_penalty=0.0):\n"
    "    \"\"\"Returns (round_df, segment_df). One row per team / per (team x segment).\n"
    "    u_outside=None matches the live game behaviour (100% demand allocated to teams).\n"
    "    Pass a float (e.g. -3.0) to model a 'no-purchase' outside option.\"\"\"\n"
...
cells.append(code(
    ...
    "# Build per-category picks; filter out categories with no suppliers in catalogue\n"
    "_pairs = [\n"
    "    (_pick_one(c, 0), _pick_one(c, 1))\n"
    "    for c in CATEGORIES\n"
    "    if _pick_one(c, 0) is not None  # category exists in catalogue\n"
    "]\n"
    "# For dirty: fall back to clean supplier when no flagged one exists in the category\n"
