...
cells.append(md(
    ...
    "5. **Penalty Sweep** — Clean vs Dirty team, penalty rate 0→100, shows profit divergence"
))
...
cells.append(code(
    ...
    "    if sel.empty: return {}\n"
    "    cols = ['env_risk','social_risk','cost_score','strategic','improvement','low_quality']\n"
    "    # child_labor / banned_chem may not exist in older Excel versions\n"
    "    for c in ('child_labor', 'banned_chem'):\n"
    "        if c in suppliers_df.columns: cols.append(c)\n"
    ...
    "        eff    = demand * N\n"
    "        penalty = (cl_penalty * a.get('child_labor', 0)\n"
    "                   + bc_penalty * a.get('banned_chem', 0))\n"
    "        margin = price - GAME_SETTINGS.cost_scale * a.get('cost_score', 0) - penalty\n"
    "        team_rows.append({\n"
    ...
    "            'penalty_per_unit':  round(penalty, 3),\n"
    "        })\n"
    "\n"
    "    seg_rows = []\n"
    ...
    "        # Round-level penalty override (if present in spec) takes precedence\n"
    "        r_cl = rnd.get('cl_penalty', cl_penalty)\n"
    "        r_bc = rnd.get('bc_penalty', bc_penalty)\n"
    "        rdf, sdf = run_round(specs, alpha=av, beta=bv, delta=d,\n"
    "                             cl_penalty=r_cl, bc_penalty=r_bc)\n"
...
# ── Scenario 5 — Penalty Sweep ────────────────────────────────────────────

cells.append(md(
    "## Scenario 5 — Penalty Sweep\n\n"
    "Two teams compete at the **same price** and similar quality.\n"
    "**Team Clean** picks suppliers with no child-labor or banned-chemical flags.\n"
    "**Team Dirty** picks at least one supplier flagged on both dimensions.\n\n"
    "The admin raises the `child_labor_penalty` and `banned_chem_penalty` each round "
    "(0 → 10 → 25 → 50 → 100 per flagged unit share). "
    "The simulation shows how profit diverges even when demand share is identical.\n\n"
    "> Note: this scenario uses *synthetic picks* derived dynamically from the loaded\n"
    "> `suppliers_df` — it auto-selects a flagged and a clean supplier from each category.\n"
    "> If the Excel does not contain flagged suppliers the scenario reduces to equal profits."
))

cells.append(code(
    "# ── Build clean vs flagged picks from real catalogue ──────────────────\n"
    "PENALTY_ROUNDS = [0, 10, 25, 50, 75, 100]\n"
    "\n"
    "def _pick_one(cat, flag_val):\n"
    "    \"\"\"Return one supplier_id from `cat` where child_labor+banned_chem == flag_val.\"\"\"\n"
    ...
    "            return str(row['supplier_id'])\n"
    "    return str(sub.iloc[0]['supplier_id'])  # fallback: any\n"
    "\n"
    "CATEGORIES = ['camera', 'keyboard', 'cable']\n"
    "_clean_picks  = [_pick_one(c, 0) for c in CATEGORIES]\n"
    "_dirty_picks  = [_pick_one(c, 1) for c in CATEGORIES]\n"
    "# For Dirty: use clean fallback if no flagged supplier in that category\n"
    "_dirty_picks = [d if d else cl for d, cl in zip(_dirty_picks, _clean_picks)]\n"
    "_clean_picks  = [p for p in _clean_picks if p]\n"
    "_dirty_picks  = [p for p in _dirty_picks if p]\n"
    "print(f'Clean picks : {_clean_picks}')\n"
    "print(f'Dirty picks : {_dirty_picks}')\n"
    "avg_cl_clean = avg_attrs(_clean_picks).get('child_labor', 0)\n"
    "avg_cl_dirty = avg_attrs(_dirty_picks).get('child_labor', 0)\n"
    "avg_bc_dirty = avg_attrs(_dirty_picks).get('banned_chem', 0)\n"
    "print(f'avg_child_labor  clean={avg_cl_clean:.3f}  dirty={avg_cl_dirty:.3f}')\n"
    "print(f'avg_banned_chem  dirty={avg_bc_dirty:.3f}')\n"
))

cells.append(code(
    "# ── Run penalty sweep ──────────────────────────────────────────────────\n"
    "S5_SPECS_BASE = [\n"
    "    {'name': 'Clean',  'picks': _clean_picks, 'price': 100},\n"
    "    {'name': 'Dirty',  'picks': _dirty_picks, 'price': 100},\n"
    "]\n"
    "\n"
    "S5 = [\n"
    "    {'round_no': i+1,\n"
    "     'teams': S5_SPECS_BASE,\n"
    "     'cl_penalty': p, 'bc_penalty': p}\n"
    "    for i, p in enumerate(PENALTY_ROUNDS)\n"
    "]\n"
    "\n"
    "s5_r, s5_s = run_scenario('S5_PenaltySweep', S5)\n"
    "print(f'S5: {len(s5_r)} round rows')\n"
    "s5_r[['round_no','team','avg_child_labor','avg_banned_chem',\n"
    "       'penalty_per_unit','unit_margin','realized_profit']]\n"
))

cells.append(code(
    "# ── Penalty sweep visualization ────────────────────────────────────────\n"
    "fig5, axes5 = plt.subplots(1, 3, figsize=(15, 5))\n"
    "fig5.suptitle('S5 — Penalty Sweep: Clean vs Dirty Supplier Strategy',\n"
    "              fontsize=13, fontweight='bold')\n"
    "\n"
    "teams5 = sorted(s5_r['team'].unique())\n"
    "colors5 = team_colors(teams5)\n"
    "penalty_vals = s5_r.drop_duplicates('round_no').sort_values('round_no')['round_no'].map(\n"
    "    dict(enumerate(PENALTY_ROUNDS, 1))\n"
    ").values\n"
    "\n"
    "for team in teams5:\n"
    "    td = s5_r[s5_r['team']==team].sort_values('round_no')\n"
    "    axes5[0].plot(PENALTY_ROUNDS, td['penalty_per_unit'].values,\n"
    "                 marker='o', label=team, color=colors5[team], linewidth=2)\n"
    "    axes5[1].plot(PENALTY_ROUNDS, td['unit_margin'].values,\n"
    "                 marker='s', label=team, color=colors5[team], linewidth=2)\n"
    "    axes5[2].plot(PENALTY_ROUNDS, td['realized_profit'].values,\n"
    "                 marker='^', label=team, color=colors5[team], linewidth=2)\n"
    "\n"
    "for ax, title, ylabel in zip(axes5,\n"
    "    ['Penalty / Unit vs CL+BC Rate', 'Unit Margin vs Rate', 'Realized Profit vs Rate'],\n"
    "    ['Penalty / Unit', 'Unit Margin', 'Realized Profit']):\n"
    "    ax.set_title(title, fontsize=10)\n"
    "    ax.set_xlabel('Penalty Rate (CL = BC = rate)')\n"
    "    ax.set_ylabel(ylabel)\n"
    "    ax.legend(fontsize=9); ax.grid(alpha=0.3)\n"
    "    ax.axhline(0, color='gray', linestyle=':', linewidth=1)\n"
    "\n"
    "plt.tight_layout()\n"
    "plt.savefig('s5_penalty_sweep.png', dpi=120, bbox_inches='tight'); plt.show()\n"
    "print('S5 penalty chart done.')\n"
))
...
cells.append(code(
    ...
    "    s5_r.to_excel(w, sheet_name='S5_Rounds',      index=False)\n"
    "    s5_s.to_excel(w, sheet_name='S5_Segments',    index=False)\n"
    "    # Pivots\n"
